    ...
    public class Logging
    {
        ...
        public static async void Log(LogLevel level, string source, string message, Exception exception = null)
        {
            try
            {
                ...
                string logText;
                if (exception == null)
                    logText = $"[{DateTime.Now.ToString()}] [{level.ToString()}] <{source}> \"{message}\"";
                else
                    logText = $"[{DateTime.Now.ToString()}] [{level.ToString()}] <{source}> \"{message}\" \"{exception.Message}\"";
        ...
        public static void CleanOldLogs()
        {
            ...
            {
                Logging.Log(LogLevel.ERROR, $"{nameof(AutoMarkCheck)}.{nameof(Logging)}.{nameof(CleanOldLogs)}", "Failed to clean old log files.", ex);
            }
        }
    }
}
